...
#测试设备ID b5e596067d2b 3df01268
devices='3df01268' 
#测试应用包名称
packages ="com.jixiaoclean.jixiao"
 #测试应用Activity名称
app_bundle_id = 'com.jixiaoclean.jixiao.ui.activity.SplashActivity'
#热启动和冷启动测试次数，推荐10次
a=10
...
b=30
#本地adb路径,默认不改
adb=os.getcwd()+'\\adb.exe'
class Utils(object):
    # 根据包名获取Pid
    @staticmethod
    def get_pid_by_package_name(packages):
        pid = os.popen(adb +' -s ' +  devices +' shell pidof '+packages).read()
        #print(pid)
        return pid
...
class App(object):
    ...
    def launch_app1(self):
        cmd = adb +' -s ' +  devices +' shell am start -W -n '+packages+'/'+app_bundle_id  # type: str
    ...
    def stop_app():
        ...
        # 热启动停止的命令,启动完至于后台
        cmd = adb +' -s ' +  devices +' shell input keyevent 3'
        os.popen(cmd)
 # 停止app，冷启动
    @staticmethod
    def stop_app2():
        # 冷启动停止的命令：完全杀进程，再启动
        cmd =adb +' -s ' +  devices +' shell am force-stop '+packages
        # 热启动停止的命令,启动完至于后台
...
#单位KB 占用总内存
    ...
    def get_mem_info(self):
        ...
        while 5 != 0:
            try:
                # 测试应用占用总内存
                native_info = os.popen(adb +' -s ' +  devices +' shell dumpsys meminfo '+packages+' | findstr "TOTAL"').read()
                print('占用总内存--原数据',native_info)
                native_pss = format(int(re.findall(r"\d+", native_info)[0]) / 1000.0, ".1f")
                # 当前手机剩余内存
                
                dalvik_info = os.popen(adb +' -s ' +  devices +' shell dumpsys meminfo  | findstr "Free"').read()
                print('剩余内存--原数据',dalvik_info)# 运行1-2分钟会有空值
                if not dalvik_info :
                    print("空值")
    ...
    def launch_app3(self):    
        ...
        #print('1测试',kj)
        kk2=str(kj)
        net_command = adb +' -s ' +  devices +' shell cat /proc/'+kk2+'/net/dev | findstr "wlan"'
        while 5 != 0:
            if self.is_first:
...
class Controller(object):
    ...
    def test_process(self):
        ...
        self.app.launch_app4()#获取电量
        self.app.launch_app11()#内存
        self.app.launch_app2()#cpu
        self.app.launch_app3()#获取流量
...
if __name__ == '__main__':
    ...
    #controller.run2()
    #pid = os.popen( adb +' -s ' +  devices +' shell am start -W -n '+packages+'/'+app_bundle_id).read()#启动应用，防止没手动启动应用，无法获取性能数据
